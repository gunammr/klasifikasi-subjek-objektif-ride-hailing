...
def show_input_ulasan():
    MODEL_PATH = Path("model/svc_model_norm1.pkl")
    VECTORIZER_PATH = Path("model/tfidf_vectorizer_norm1.pkl")
    CSV_PATH = Path("dataset/hasil_prediksi.csv")

    # Proteksi jika file model tidak ditemukan
    if not MODEL_PATH.exists() or not VECTORIZER_PATH.exists():
        st.error(f"File model tidak ditemukan! Pastikan model diletakkan di: {MODEL_PATH.absolute()}")
        return
    ...
    with tab1:
        ...
        if prediksi_btn:
            ...
            if ulasan.strip() == "":
                ...
                ulasan_asli = ulasan
                ulasan = preprocess_review(ulasan)
            ...
            if "review" not in df_upload.columns:
                ...
                if st.button(
                    ...
                    for review in df_upload["review"]:
                        ...
                        review = preprocess_review(review_asli)
